...
for attempt in 1 2 3; do
    wget -q -O /root/beszel-install.sh https://raw.githubusercontent.com/PetroSky-Cloud/One-click-app/main/beszel.sh && [ -s /root/beszel-install.sh ] && break
    sleep 5
done
...
[ -s /root/beszel-install.sh ] && bash /root/beszel-install.sh
STUB
...
echo "[$(date)] Beszel Installation Started"
...
echo "[$(date)] Beszel init complete - run install.sh on first SSH login"
{/literal}
