...
for attempt in 1 2 3; do
    wget -q -O /root/openwebui-install.sh https://raw.githubusercontent.com/PetroSky-Cloud/One-click-app/main/openwebui.sh && [ -s /root/openwebui-install.sh ] && break
    sleep 5
done
...
[ -s /root/openwebui-install.sh ] && bash /root/openwebui-install.sh
STUB
...
echo "[$(date)] Open WebUI Installation Started"
...
echo "[$(date)] Open WebUI init complete - run install.sh on first SSH login"
{/literal}
