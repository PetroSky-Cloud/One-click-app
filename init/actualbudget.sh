...
for attempt in 1 2 3; do
    wget -q -O /root/actualbudget-install.sh https://raw.githubusercontent.com/PetroSky-Cloud/One-click-app/main/actualbudget.sh && [ -s /root/actualbudget-install.sh ] && break
    sleep 5
done
...
[ -s /root/actualbudget-install.sh ] && bash /root/actualbudget-install.sh
STUB
...
echo "[$(date)] Actual Budget Installation Started"
...
echo "[$(date)] Actual Budget init complete - run install.sh on first SSH login"
{/literal}
