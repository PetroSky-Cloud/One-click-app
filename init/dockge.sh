...
for attempt in 1 2 3; do
    wget -q -O /root/dockge-install.sh https://raw.githubusercontent.com/PetroSky-Cloud/One-click-app/main/dockge.sh && [ -s /root/dockge-install.sh ] && break
    sleep 5
done
...
[ -s /root/dockge-install.sh ] && bash /root/dockge-install.sh
STUB
...
echo "[$(date)] Dockge Installation Started"
...
echo "[$(date)] Dockge init complete - run install.sh on first SSH login"
{/literal}
