...
for attempt in 1 2 3; do
    wget -q -O /root/grafana-prometheus-install.sh https://raw.githubusercontent.com/PetroSky-Cloud/One-click-app/main/grafana-prometheus.sh && [ -s /root/grafana-prometheus-install.sh ] && break
    sleep 5
done
...
[ -s /root/grafana-prometheus-install.sh ] && bash /root/grafana-prometheus-install.sh
STUB
...
echo "[$(date)] Grafana + Prometheus Installation Started"
