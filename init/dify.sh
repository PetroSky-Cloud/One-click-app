...
apt-get update
apt-get install -y wget bash curl net-tools git

# Download installer with retry - a failed wget must not leave a silent 0-byte file
for attempt in 1 2 3; do
    wget -q -O /root/dify-install.sh https://raw.githubusercontent.com/PetroSky-Cloud/One-click-app/main/dify.sh && [ -s /root/dify-install.sh ] && break
    sleep 5
done
...
[ -s /root/dify-install.sh ] && bash /root/dify-install.sh
STUB
...
echo "[$(date)] Dify Installation Started"
...
echo "[$(date)] Dify init complete - run install.sh on first SSH login"
{/literal}
